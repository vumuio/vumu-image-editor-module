...
export default class Image extends Component {
  constructor(graphics) {
    super(componentNames.IMAGE, graphics);
   ...
   * @param {fabric.Object} imageObject - fabric object
   * @returns {Object}
   */
  makeFillPropertyForUserEvent(imageObject) {
    const fillType = getFillTypeFromObject(imageObject);
    const fillProp = {};

    if (fillType === SHAPE_FILL_TYPE.FILTER) {
      const fillImage = getFillImageFromShape(imageObject);
      ...
      fillProp.color = imageObject.fill || 'transparent';
    }
   ...
   * @param {fabric.Object} imageObject - Shape object
   * @param {fabric.Object} originalShapeObj - Shape object
   */
  processForCopiedObject(imageObject, originalShapeObj) {
    this._bindEventOnShape(imageObject);

    if (getFillTypeFromObject(imageObject) === 'filter') {
  ...
  _bindEventOnShape(imageObject) {
    ...
    imageObject.on({
      ...
      modifiedInGroup(activeSelection) {
        self._fillFilterRePositionInGroupSelection(imageObject, activeSelection);
      },
      moving() {
   ...
   * @private
   */
  _resetPositionFillFilter(imageObject) {
    if (getFillTypeFromObject(imageObject) !== 'filter') {
      return;
    }

    const { patternSourceCanvas } = getCustomProperty(imageObject, 'patternSourceCanvas');

    const fillImage = getFillImageFromShape(imageObject);
    const { originalAngle } = getCustomProperty(fillImage, 'originalAngle');

    if (this.graphics.canvasImage.angle !== originalAngle) {
      reMakePatternImageSource(imageObject, this.graphics.canvasImage);
    }
    const { originX, originY } = imageObject;

    resizeHelper.adjustOriginToCenter(imageObject);

    imageObject.width *= imageObject.scaleX;
    imageObject.height *= imageObject.scaleY;
    // imageObject.rx *= imageObject.scaleX;
    // imageObject.ry *= imageObject.scaleY;
    imageObject.scaleX = 1;
    imageObject.scaleY = 1;

    rePositionFilterTypeFillImage(imageObject);

    changeOrigin(imageObject, {
      originX,
      originY,
    });
   ...
   * @private
   */
  _fillFilterRePositionInGroupSelection(imageObject, activeSelection) {
    ...
    const { angle, left, top } = imageObject;

    fabric.util.addTransformToObject(imageObject, activeSelection.calcTransformMatrix());
    this._resetPositionFillFilter(imageObject);
